...
date: "2025-06-21"
categories: ["Desenvolvimento", "São Paulo", "IDH"]
...
atlas_sp <- readr::read_rds(here::here(
  "insights/posts/2025-06-wz-sp-idh-atlas",
  "atlas_sp_idh_2010.rds"
))
```
...
# Uma cor por faixa: a primeira da rampa não entra no gráfico.
ek_breaks <- seq(0.6, 0.95, 0.05)
ek_bins <- ek_blue[-1]

# scale_fill_stepsn() tinge cada bin com a rampa amostrada no midpoint do
# bin, em escala rescalada (break rescalado - diff/2). Fixando `values`
# nesses mesmos midpoints, cada bin recebe a cor exata de ek_bins — as
# mesmas que o scale_fill_manual() das colunas usa, sem interpolação.
ek_limits <- c(0.6, max(atlas_sp$idhm))
ek_mids <- local({
  brk <- sort(unique(c(ek_limits[1], ek_breaks, ek_limits[2])))
  brk <- scales::rescale(brk, from = ek_limits)
  brk[-1] - diff(brk) / 2
})
...
pmap <- ggplot(atlas_sp) +
  ...
  labs(
    ...
    caption = "Fonte: Atlas Brasil (Ipea/FJP), Censo Demográfico 2010 • EKIO"
  ) +
  theme_ekio_site(grid = "none", ticks = "none", font_title = "Lato") +
  theme(
...
pop_idh <- atlas_sp |>
  st_drop_geometry() |>
  mutate(group_idh = findInterval(idhm, ek_breaks)) |>
...
pcol <- ggplot(pop_idh, aes(group_idh, share, fill = as.factor(group_idh))) +
  ...
  scale_fill_manual(values = ek_bins) +
...
# O "&" pinta o canvas externo do patchwork (senão ele pinta branco por cima
# do dispositivo) e o dev.args em _ekio-style.R cobre o resto do dispositivo.
p_idh_atlas <-
  pmap +
  inset_element(pcol, left = 0.50, bottom = 0.05, right = 1, top = 0.5) &
...
```{r}
#| fig-alt: "Mapa do IDH das Unidades de Desenvolvimento Humano de São Paulo, com dados do Censo 2010. Tons mais escuros indicam IDH maior, concentrado no quadrante sudoeste. Um gráfico de colunas inserido no mapa mostra a participação da população por faixa de IDH."
#| out-width: "100%"
#| fig-width: 9
#| fig-height: 12
#| fig-dpi: 72
