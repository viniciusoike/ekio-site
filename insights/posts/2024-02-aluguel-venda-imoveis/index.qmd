...
date: "2024-02-21"
categories: ["Mercado Imobiliário", "Economia", "Aluguel"]
description: "Os mercados de aluguel e de venda de imóveis são interligados e tendem a um equilíbrio. As séries do FipeZap indicam uma quebra nessa tendência histórica e o começo de um novo ciclo de aluguel."
image: "/static/images/art/hokusai-series-2026-09/card-positive-correlation.webp"
image-alt: "Ilustração de dispersão com correlação positiva e uma reta ajustada"
---
...
```{r import-data}
# Post published 2024-02-21. Source series carry roughly a two-month
...
fipe_changes <- fipezap |>
  ...
  mutate(
    cycle = factor(case_when(
      date >= as.Date("2008-01-01") & date <= as.Date("2012-06-01") ~
        "Commodity Cycle",
      date >= as.Date("2014-06-01") & date <= as.Date("2017-12-01") ~
        "Dilma Recession",
...
p3 <- ggplot(
  ...
  geom_smooth(method = "gam", se = FALSE, color = ekio_pal("full")[[1]]) +
  geom_hline(yintercept = 0, color = ekio_pal("gray")[5], linewidth = 0.4) +
  geom_vline(xintercept = 0, color = ekio_pal("gray")[5], linewidth = 0.4) +
  labs(
    x = "Venda (var. %)",
    y = "Aluguel (var. %)",
  ...
  scale_color_ekio_d("full", name = "Ciclo Econômico") +
  theme_ekio_site()
...
Vale notar que o mercado de aluguel é bastante lento. A "migração" do aluguel para um imóvel próprio não é rápida nem simples. Além do custo monetário, há diversos custos de transação envolvidos no processo. De fato, em muitos casos, não é possível fazer uma migração entre um e outro por motivos contratuais.

Por fim, a literatura empírica do tema não é unânime sobre a relação entre os mercados de venda e de aluguel. Um estudo sobre o [mercado de Cingapura encontrou evidência de cointegração apenas em regiões específicas](https://www.sciencedirect.com/science/article/abs/pii/S0166046215000800) e não conseguiu estabelecer a presença de cointegração em nível nacional.

## Os dados

As fontes de dados para verificar a dinâmica destes mercados são limitadas no Brasil. O FipeZap oferece a série de aluguel e de venda mais extensa. Este índice coleta todos os anúncios online de imóveis publicados no sistema ZapImóveis e constrói um índice de mediana. Os dados são estratificados por número de dormitórios e territorialmente, usando regiões que se baseiam nas áreas de ponderação do IBGE.

O gráfico abaixo mostra as séries de aluguel e de venda (residenciais) em nível nacional, agregadas por tipologia. Como se vê, o comportamento das séries é muito similar no período 2008-2015. Durante a Crise Econômica, o mercado de aluguel parece sofrer mais do que o mercado de venda. Durante e após a pandemia do Covid-19, ambos os mercados se recuperam com força, mas o mercado de aluguel parece crescer mais.

```{r plot-rent-sale}
#| fig-alt: "Gráfico de linhas dos índices FipeZap de aluguel e venda, normalizados pela média de 2019. As séries seguem trajetórias semelhantes até meados da década de 2010 e divergem durante a recessão e a pandemia."
p1
```

Na variação acumulada do período, vê-se que o mercado de venda acumulou uma alta significativamente maior do que a do mercado de aluguel. A divergência parece ter começado em 2009. A diferença entre as séries fica estável durante a Crise Econômica e o mercado de aluguel parece começar a reduzir esta distância no período da pandemia.

```{r plot-rent-sale-acum}
#| fig-alt: "Gráfico de linhas da variação acumulada dos preços de aluguel e venda desde o início da série. Os preços de venda acumulam alta maior; a distância começa a diminuir com a recuperação dos aluguéis após a pandemia."
p2
```
...
#| fig-alt: "Gráfico de dispersão das variações mensais dos preços de aluguel e venda, com curvas de ajuste e cores por ciclo econômico. A relação entre as duas variações muda no período da pandemia e pós-pandemia."
p3
```
...
#| fig-alt: "Gráfico de dispersão das variações mensais dos preços de aluguel e venda, com retas de regressão por ciclo econômico. O período da pandemia e pós-pandemia se distingue pela relação entre as duas séries."
p4
```

## Um novo ciclo de aluguel?

Usando uma abordagem mais rigorosa, podemos buscar algum tipo de cointegração entre as séries. Pelos testes de raiz unitária usuais, é fácil concluir que ambas as séries são I(1)[^1]. Contudo, tanto o teste de Johansen como o teste de Engle-Granger apontam que não há relação de cointegração entre as séries[^2].

[^1]: Usou-se o teste de Phillips-Perron (`ur.pp`) e o teste de Dickey-Fuller aumentado (`ur.df`) do pacote `urca`. Na série de venda, incluiu-se uma constante e uma tendência linear. Na série de aluguel não se incluiu nem constante nem tendência linear.
...
```{r series-ivar-fipe}
#| fig-alt: "Gráfico de linhas do FipeZap de aluguel e do IVAR, com janeiro de 2019 igual a 100. Durante a pandemia, o IVAR registra quedas mais acentuadas. Ambos sobem a partir de 2022, mas o FipeZap acumula alta maior."
p_fipe_ivar
```

Não é fácil afirmar que um ou outro índice seja mais correto. Contudo, vale afirmar que a metodologia do IVAR é considerada superior à metodologia do FipeZap segundo as melhores práticas internacionais. Ainda assim, é importante notar que a base territorial do FipeZap é consideravelmente superior: na sua versão mais recente, o índice abarca mais de 50 cidades; o IVAR, por outro lado, está disponível somente para quatro cidades (Belo Horizonte, Porto Alegre, Rio de Janeiro e São Paulo). De qualquer maneira, a série histórica do IVAR é muito recente; o índice remonta apenas a 2019, dificultando qualquer tipo de análise de cointegração.
