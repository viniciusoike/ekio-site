...
data_cutoff <- as.Date("2024-04-01")
...
p_rppi_comp <- ggplot(rppi_comp, aes(date, new_index, color = source)) +
  ...
  labs(
    ...
    subtitle = str_wrap(
      "Comparação de índices de preços imobiliários e inflação geral. Todos os índices normalizados para jan/2014.",
      111
    ),
    x = NULL,
    y = "Índice (2019 = 100)",
...
p_rppi_brasil <- ggplot(
  ...
  labs(
    title = "Índices de Preços Imobiliários no Brasil",
    subtitle = str_wrap(
      "Comparação de índices de preços imobiliários no Brasil. Todos os índices normalizados para jan/2014.",
      111
    ),
    x = NULL,
    y = "Índice (jan/14 = 100)",
...
p_rppi_incc <-
  ...
  scale_color_ekio_d("full", name = "") +
  labs(
    # Measures 8.2 in at 18pt Lora, wider than the 7.85 in an 8 in canvas
    # leaves. Wrapped rather than cut.
    title = "Inflação de Custos supera valorização\nimobiliária nos últimos dez anos",
    subtitle = str_wrap(
      "Comparação de índices de preços imobiliários e do INCC (Índice Nacional de Custos da Construção Civil). Todos os índices normalizados para jan/2014.",
      111
    ),
    x = NULL,
    y = "Índice (jan/14 = 100)",
...
p_ivgr_ipca <- ggplot(
  ...
  labs(
    ...
    subtitle = str_wrap(
      "Comparação de índices de preços: IVG-R, INCC e IPCA. Todos os índices normalizados para mar/2001.",
      111
    ),
    x = NULL,
    y = "Índice (mar/01 = 100)",
...
plot_rppi <- function(name, start_date = NULL) {
  ...
  if (is.null(start_date)) {
    ...
    start_date <- format(start_date, "%Y/%b")
  }
  ...
  p_index <- ggplot(filter(rppi_sale, source == name), aes(date, index)) +
    ...
    labs(
      x = NULL,
      y = NULL,
      subtitle = str_glue("Índice (100 = {start_date})")
    ) +
    theme_ekio_site()

  p_12m <- ggplot(filter(rppi_sale, source == name), aes(date, acum12m)) +
    geom_line(linewidth = 0.8, color = ekio_pal("full")[[1]]) +
    geom_hline(yintercept = 0, color = ekio_pal("gray")[5], linewidth = 0.4) +
    scale_y_continuous(labels = scales::label_percent()) +
    labs(x = NULL, y = NULL, subtitle = "Acumulado 12 meses") +
    theme_ekio_site()
  ...
  p_chg <- ggplot(filter(rppi_sale, source == name), aes(date, chg)) +
    ...
    geom_hline(yintercept = 0, color = ekio_pal("gray")[5], linewidth = 0.4) +
    scale_y_continuous(labels = scales::label_percent()) +
    labs(x = NULL, y = NULL, subtitle = "Variação Mensal") +
    theme_ekio_site()
...
```{r}
#| fig-alt: "Três gráficos de linhas do IVG-R: nível do índice, variação em 12 meses e variação mensal. Os painéis mostram a valorização no longo prazo e as oscilações do ritmo de crescimento."
#| fig-width: 8
#| fig-height: 11
#| out-width: "100%"
plot_rppi("IVG-R")
...
```{r}
#| fig-alt: "Três gráficos de linhas do FipeZap: nível do índice, variação em 12 meses e variação mensal. Os painéis distinguem a trajetória dos preços das mudanças no ritmo de valorização."
#| fig-width: 8
#| fig-height: 11
#| out-width: "100%"
plot_rppi("FipeZap")
...
```{r}
#| fig-alt: "Três gráficos de linhas do IGMI-R: nível do índice, variação em 12 meses e variação mensal. Os painéis mostram a aceleração dos preços após 2020."
#| fig-width: 8
#| fig-height: 11
#| out-width: "100%"
plot_rppi("IGMI-R")
