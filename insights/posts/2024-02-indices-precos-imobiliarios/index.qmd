...
theme_vini <- theme_minimal(base_family = "Lato", base_size = 11) +
  theme(
    panel.grid.minor = element_blank(),
    panel.background = element_rect(fill = col_offwhite, color = col_offwhite),
    plot.background = element_rect(fill = col_offwhite, color = col_offwhite),
...
sale <- subset(sale, date <= as.Date("2023-12-31"))
```
...
```{r vecm-analysis}
tbl_rppi = rppi_comp |>
  filter(date >= as.Date("2014-01-01"), date < as.Date("2024-01-01")) |>
  pivot_wider(
    id_cols = "date",
    names_from = "source",
