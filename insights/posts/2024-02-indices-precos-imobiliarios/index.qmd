...
p_rppi_comp <- ggplot(rppi_comp, aes(date, new_index, color = source)) +
  geom_hline(yintercept = 100, color = ekio_rule_color, linewidth = 0.4) +
  geom_line(lwd = 0.8) +
...
plot_rppi <- function(name) {
  ...
  p_12m <- ggplot(filter(rppi_sale, source == name), aes(date, acum12m)) +
    geom_line(lwd = 0.8, color = ekio_pal("full")[[1]]) +
    geom_hline(yintercept = 0, color = ekio_rule_color, linewidth = 0.4) +
