...
bis <- bis %>%
  mutate(
    name_country = case_when(
      ...
      ref_area_name == "World" ~ "Média Mundo",
...
p_bis_geral <- ggplot(rppi_comp, aes(x = date, y = value)) +
  ...
  labs(
    ...
    subtitle = str_wrap(
      "Índices de preços imobiliários reais (ajustados pela inflação local) indexados nos valores médios de 2010. Definições de Avançados e Emergentes seguem convenções do BIS.",
      111
    ),
...
p_bis_latam <-
  ...
  labs(
    ...
    subtitle = str_wrap(
      "Índices de preços imobiliários reais (ajustados pela inflação local) indexados nos valores médios de 2010. Comparação do Brasil com outros países selecionados.",
      111
    )
  ) +
  theme_ekio_site() +
...
Segundo os dados do Bank for International Settlements (BIS).
