---
title: "Housing Affordability em São Paulo"
date: "2022-03-07"
date-modified: "2026-09-10"
categories: ["Mercado Imobiliário", "Acessibilidade", "São Paulo"]
description: "O Price-Income Ratio de São Paulo ficou próximo de 20 em 2021. Entre janeiro de 2020 e fevereiro de 2024, os preços dos imóveis na cidade subiram quase 80%, acima da inflação."
image: "/static/images/art/hokusai-series-2026-09/insight-compact-housing.webp"
image-alt: "Ilustração de casas compactas e sobrados justapostos"
---

```{r}
#| include: false
library(ggplot2)
library(dplyr)
library(sf)
library(realestatebr)
library(ekioplot)
import::from(stringr, str_wrap)
source(here::here("insights/posts/_ekio-style.R"))

# Dados vendorizados junto ao post (relativo ao diretório do .qmd)
affordability <- st_read(
  "affordability_relatorio_local.geojson",
  quiet = TRUE
)

affordability <- affordability |>
  mutate(
    inc_med_label = round(inc_med_total, -2),
    pc_m2_med_label = round(pc_m2_med, -2)
  )
```

```{r}
jenks <- BAMMtools::getJenksBreaks(affordability$pir_med_total, k = 8)

p_map <- ggplot(affordability) +
  geom_sf(aes(fill = pir_med_total), linewidth = 0.15, color = "white") +
  scale_fill_stepsn(
    name = "",
    colours = as.character(ekio_pal("red")),
    breaks = jenks,
    labels = round(jenks),
    na.value = ekio_pal("gray")[2]
  ) +
  labs(
    title = "Housing Affordability",
    subtitle = "Distribuição do Price-Income Ratio em São Paulo"
  ) +
  theme_ekio_site() +
  theme(
    axis.line.x = element_blank(),
    axis.ticks.x = element_blank(),
    axis.text.x = element_blank(),
    axis.text.y = element_blank(),
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
    panel.grid.major.y = element_blank(),
    legend.position = "top",
    legend.justification = 0.5,
    legend.key.size = unit(0.5, "cm"),
    legend.key.width = unit(2, "cm"),
    legend.text = element_text(size = 12),
    legend.margin = margin(),
    plot.margin = margin(10, 5, 5, 10),
    plot.title = element_text(size = 32, hjust = 0.5),
    plot.subtitle = element_text(size = 12, hjust = 0.5)
  )
```

Quantos anos de renda familiar são necessários para comprar um imóvel típico em São Paulo? Em 2021, a resposta ficava em torno de vinte, segundo a estimativa apresentada a seguir. Entre janeiro de 2020 e fevereiro de 2024, os preços dos imóveis na cidade subiram quase 80%, enquanto a inflação acumulou cerca de 29%.

## O PIR em São Paulo

O mapa abaixo apresenta o Price-Income-Ratio (PIR) por Zona OD na cidade de São Paulo. Os dados são de 2021 e provêm do artigo [Acesso à moradia em São Paulo: visão geral e mensuração](https://lares.architexturez.net/system/files/4DRB.pdf), publicado na LARES 2021.

O PIR é um indicador simples que mensura a acessibilidade financeira à moradia numa região: sua interpretação é do tipo "quanto maior, pior", isto é, quanto maior for o valor do PIR, pior é a acessibilidade financeira à moradia. Especificamente, ele mede a razão entre os preços dos imóveis disponíveis em uma região (preços de anúncios) e a renda média das famílias na cidade.

O mapa abaixo compara o preço médio dos imóveis disponíveis nestas regiões com a renda média familiar bruta de São Paulo. Regiões centrais como Jardins e Alto de Pinheiros apresentam os piores indicadores de acessibilidade. No mapa, o PIR varia de cerca de 3 anos de renda em bairros da periferia leste, como Cidade Tiradentes e Santa Etelvina, a quase 29 anos no Jardim Europa; na metade das zonas, o indicador fica entre 6 e 12 anos. As zonas em cinza não possuem dados de preço disponíveis para o período.

```{r}
#| fig-alt: "Mapa do Price-Income Ratio por Zona OD em São Paulo em 2021. As cores representam o preço dos imóveis em anos de renda familiar. Os maiores valores se concentram nos Jardins e em Alto de Pinheiros; áreas sem dados aparecem em cinza."
#| out-width: "100%"
#| fig-width: 9
#| fig-height: 12
#| fig-dpi: 72

p_map
```

## O que é o PIR?

O PIR é uma razão simples entre o preço médio/mediano dos imóveis e a renda média/mediana anual das famílias:

$$\text{PIR} = \frac{\text{Preços}}{\text{Renda Anual}}$$

O PIR indica, grosso modo, a quantidade de "anos de trabalho" que uma família típica precisa investir de renda para comprar um imóvel típico. A principal vantagem do PIR é a sua simplicidade de cálculo, o que facilita comparações entre diferentes regiões. A principal desvantagem do PIR é ignorar as condições de financiamento disponíveis para a população. Supondo que a renda anual média das famílias seja $R\$30.000$ e o preço médio dos imóveis, $R\$270.000$, o valor do PIR seria:

$$\text{PIR} = \frac{R\$270.000}{R\$30.000} = 9$$

Na literatura internacional, valores de PIR na faixa de 5-10 indicam baixa acessibilidade financeira. Estudos feitos em países em desenvolvimento e subdesenvolvidos, contudo, apontam valores de PIR muito mais elevados do que este intervalo, devido tanto à **baixa renda média da população quanto às condições limitadas de acesso ao crédito**. Neste sentido, talvez uma comparação mais justa seja com as megacidades chinesas, que têm PIR na faixa de 10-15. No artigo [Acesso à moradia em São Paulo: visão geral e mensuração](https://lares.architexturez.net/system/files/4DRB.pdf), escrito por Vinicius Oike, em parceria com outros colegas, estima-se que o PIR de São Paulo em 2021 era próximo de 19-20. O site [Numbeo apresenta o PIR](https://www.numbeo.com/property-investment/rankings.jsp?title=2021) para um amplo grupo de cidades no mundo: em 2021, segundo o site, o PIR de São Paulo era de 17,8.

## Preços dos imóveis

```{r}
igmi <- get_dataset("rppi", "igmi")

subigmi <- igmi |>
  filter(name_muni %in% c("Brasil", "São Paulo"), date <= as.Date("2024-02-01"))

bcb_series <- get_dataset("bcb_series", "primary")

ipca <- bcb_series |>
  filter(name_simplified == "ipca") |>
  select(date, name_simplified, value)

subipca <- ipca |>
  filter(date %in% unique(subigmi$date)) |>
  mutate(
    acum12m = RcppRoll::roll_prodr(1 + value / 100, n = 12) - 1
  )

dat <- bind_rows(subigmi, subipca)

lvls_series <- c("São Paulo", "Brasil", "ipca")
lbls_series <- c("Imóveis (São Paulo)", "Imóveis (Brasil, média)", "IPCA")

dat <- dat |>
  tidyr::unite("name_series", c(name_simplified, name_muni), na.rm = TRUE) |>
  mutate(
    name_series = factor(
      name_series,
      levels = lvls_series,
      labels = lbls_series
    )
  )

subdat <- dat |>
  filter(date >= "2015-01-01")

color_palette <- c(ekio_pal("blue")[c(5, 7)], ekio_pal("accent_blue")[2])
color_palette <- unname(color_palette)

p_acum12m <- ggplot(subdat, aes(date, acum12m)) +
  geom_line(aes(color = name_series), linewidth = 0.8) +
  geom_hline(yintercept = 0, color = ekio_pal("gray")[5], linewidth = 0.4) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(labels = scales::label_percent()) +
  scale_color_manual(
    name = NULL,
    values = color_palette
  ) +
  labs(
    title = "Aceleração dos preços dos imóveis após a Pandemia",
    subtitle = str_wrap(
      "Variação acumulada em 12 meses dos preços dos imóveis residenciais em São Paulo, no Brasil e do IPCA.",
      111
    ),
    caption = "Fonte: IGMI-R (Abecip/FGV), IPCA (IBGE) • EKIO",
    y = NULL,
    x = NULL
  ) +
  theme_ekio_site()

subdat_index <- dat |>
  filter(date >= as.Date("2020-01-01")) |>
  mutate(chg = if_else(is.na(chg), value / 100, chg)) |>
  mutate(acum = cumprod(1 + chg) - 1, .by = "name_series")

aux_label <- subdat_index |>
  slice_max(date, n = 1) |>
  mutate(
    label = scales::percent(acum, decimal.mark = ","),
    pos_x = date + months(4)
  )

p_index <- ggplot(subdat_index, aes(date, acum, group = name_series)) +
  geom_line(aes(color = name_series), linewidth = 0.8) +
  geom_label(
    data = aux_label,
    aes(x = pos_x, label = label),
    family = "Lato",
    size = 3,
    hjust = 1,
  ) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(
    labels = scales::label_percent(),
    expand = expansion(c(0, 0.05))
  ) +
  scale_color_manual(
    name = NULL,
    values = color_palette
  ) +
  labs(
    title = "Aumento de quase 80% em SP desde a Pandemia",
    subtitle = str_wrap(
      "Variação acumulada dos preços dos imóveis residenciais em São Paulo e no Brasil comparados com o IPCA.",
      111
    ),
    caption = "Fonte: IGMI-R (Abecip/FGV), IPCA (IBGE) • EKIO",
    y = NULL,
    x = NULL
  ) +
  theme_ekio_site()
```


A pandemia de Covid-19 acelerou o aumento dos preços dos imóveis no Brasil. Até então, o mercado ainda digeria os efeitos da recessão de 2015-2016: os preços acumulavam queda em 12 meses do fim de 2016 ao fim de 2017 e, na recuperação seguinte, as altas anuais não passavam de um dígito.

```{r}
#| fig-alt: "Gráfico de linhas da variação dos preços em 12 meses em São Paulo e no Brasil, comparada ao IPCA. A alta dos imóveis acelera após 2020 e perde força depois do pico de 2021."
p_acum12m
```

A pandemia criou um boom imobiliário. Entre janeiro de 2020 e fevereiro de 2024, os preços dos imóveis em São Paulo acumularam alta de quase 80% — contra 64% na média brasileira e 29% de inflação. O pico da aceleração veio no fim de 2021, quando os preços subiram mais de 20% em 12 meses; desde então, o ritmo perdeu força, mas segue bem acima da inflação.

```{r}
#| fig-alt: "Gráfico de linhas da variação acumulada entre janeiro de 2020 e fevereiro de 2024. Os preços dos imóveis sobem quase 80% em São Paulo e 64% no Brasil, enquanto o IPCA acumula cerca de 29%."
p_index
```

## Considerações finais

Os dois quadros contam, no fundo, a mesma história. Em 2021, o PIR de São Paulo já estava próximo de 20, e as regiões mais nobres registravam valores ainda mais extremos: quase 29 anos de renda no Jardim Europa e mais de 25 na rua Oscar Freire. Entre janeiro de 2020 e fevereiro de 2024, os preços acumularam alta de quase 80%, contra 29% de inflação. Esse intervalo começa antes da estimativa do PIR. Para medir a mudança na acessibilidade após 2021, seria necessário comparar preços e renda familiar no mesmo período.

## Referências

- [Acesso à moradia em São Paulo: visão geral e mensuração](https://lares.architexturez.net/system/files/4DRB.pdf)

{{< include ../../_post-cta.qmd >}}
